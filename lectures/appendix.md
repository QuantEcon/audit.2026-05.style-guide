...
|-------------|-----------|---------:|------------:|
| `qe-math-010` | `\mathbb{P}` / `\mathbb{E}` / `\mathbb{V}` (with braces) for probability, expectation, variance | **105 / 348** | 1,167 |
| `qe-writing-009` | Write "IID", not "i.i.d." / "iid" | 30 | 61 |
| `qe-math-011` | Plain letters for distribution names (`N`, not `\mathcal{N}`) | 24 | 86 |
