...
1. Replace the 7 raw URLs with reference-style links (36, 56, 86, 412, 486, 498, 501). Six of them point at `perm_income_cons`, which is a lecture in this same series - `{doc}`perm_income_cons`` - and the seventh points at `kalman`, which is `{doc}`intermediate:kalman`` under lecture-dp's intersphinx mapping. Note that the domain used, `python-intro.quantecon.org`, is not in that mapping at all, so these are also the links most likely to be dead (qe-link-002 x7).
2. Either write the code for the last three sections or stop promising it. Lines 814-848 specify a four-step simulation and 850-870 promises to recompute $\{a_t\}$ from `` {eq}`eqn_3` `` and check it against the earlier sequence; no code cell exists after line 812, so the lecture's central experiment - two consumers with identical income histories and different information - is never actually run.
3. Convert the 17 `\begin{array}` matrices at 549-597 and 686-709 to `bmatrix` (qe-math-003 x17). The lecture already uses `bmatrix` for the state-space systems at 464-479, so this is an internal inconsistency, and the `array` blocks are also the file's least readable source.
4. Fix the variance formula, which the lecture states two ways. Lines 152, 202, 261 and 496 give $\sigma_a^2 = \beta^{-2}\sigma_\epsilon^2$; lines 209 and 222 give $\beta^{-1}\sigma_\epsilon^2$. Line 222 also has an operator slip - $\sigma_\epsilon^2 + [1 + (\beta - \beta^{-1})^2\sum_j \beta^{2j}]$ should be a product, not a sum - and as printed does not equal either candidate.
5. Switch the 23 emphasis bolds listed above to italic and turn the two bold pseudo-headings at 415 and 425 into real text or headings, so that bold is left to do only what it does at 39, 158, 196 and 376 - mark definitions.
6. Clear the 52 double and triple spaces (qe-writing-008) - at that density in an 879-line file the source reads as pasted-together, and several fall inside sentences the reader must parse closely - then fix 'the the' (227), 'incomethe' (439), 'reprentation' (361), 'thid' (486), 'formulae' (525), and the missing full stops at 245 and 760.
7. Finish the figures: mystnb `name`/`caption` metadata on the 4 plotting cells (746, 766, 796, 805), the 4 `plt.title` calls moved into captions (747, 767, 801, 810), `lw=2` on the 11 line plots (748, 749, 768, 769, 770, 798, 799, 800, 807, 808, 809), and a `plt.show()` on each - as written the cells end on `plt.legend()`, whose return value is rendered above the figure. Also lower-case the 8 mid-sentence capitals (485, 486, 680, 711) and write IID at 127, 146, 149 and 481, and brace the 4 bare `E [` expectations at 234, 380, 383 and 405 (qe-math-010 (proposed), proposed).
