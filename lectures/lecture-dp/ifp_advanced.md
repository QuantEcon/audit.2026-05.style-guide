...
5. Fix the math markup: brace the 6 bare `\mathbb E` at 85, 141, 150, 152 and 157 (qe-math-010 (proposed)), and delete the raw `\label`` {a:y0}` at 158, which is LaTeX that MathJax does not honour and which the corpus convention replaces with ` ``$$ ... $$ (label)` numbering (qe-math-007 x1).
