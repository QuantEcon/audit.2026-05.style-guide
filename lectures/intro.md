...
- **Scoring dimensions:** Writing, Math, Code, Figures, References, Links, Admonitions — 7 in-scope categories (49 rules + 7 proposed addendum rules). **JAX is out of scope** (those rules apply to JAX-specific lectures concentrated in `lecture-jax`, not in this corpus).
- **Spec / rubric:** [the audit spec](spec.md) — uses canonical `qe-*` rule IDs from [QuantEcon/action-style-guide](https://github.com/QuantEcon/action-style-guide). Rules marked **(proposed)** are documented in the style guide but not yet in the rule registry; see [the contribution issues](https://github.com/QuantEcon/action-style-guide/issues/18).

This report has three layers: this cross-series synthesis, the [visual charts](charts.md), and a drill-down report for every lecture (in the sidebar, grouped by series).
...
**Headline:** the corpus averages **8.0/10** overall. **29 lectures (9.7%)** are HIGH priority — concentrated in `lecture-dp` (17) and `lecture-python-advanced.myst` (6). Most HIGH lectures are otherwise-sound files dragged down by a single weak category (most often Figures, References, or Math).
...
Issues ranked by total lecture-count across all five series. Figures and Code account for three of the top five systemic issues, alongside the long-standing Title Case heading problem.

### 1. `qe-writing-006` — Title Case in section headings (~170 / 299 lectures)
The single most prevalent violation.
- `lecture-python.myst`: 87 / 110 (79 %)
- `lecture-python-programming`: 24 / 26 (92 %)
- `lecture-dp`: 21 / 50
- `lecture-python-intro`: 19 / 51

### 2. `qe-code-002` — Greek letters spelled out (`alpha`, `beta`) instead of unicode (`α`, `β`) (~150 / 299 lectures)
...
- Smaller counts elsewhere; `lecture-python-programming` is the exemplar (100 % compliant where applicable)
...
### 4. `qe-fig-005` — Figures lack a descriptive `:name:` field for `numref` cross-referencing (~120 / 299 lectures)
...
### 7. `qe-math-010` (proposed) — `\mathbb{P/E/V}` with braces for probability, expectation, variance (~60 / 299 lectures)
...
### 11. `qe-math-011` (proposed) — `\mathcal{N}` for the normal distribution (~30 / 299 lectures)

### 12. `qe-writing-009` (proposed) — `i.i.d.` / `iid` instead of `IID` (~30 / 299 lectures)
...
`lecture-dp` has 5 lectures that install packages mid-lecture; others scattered.
...
These can break PDF builds or produce broken references. **Two such findings, both in `lecture-python.myst`:**
...
| `lecture-python.myst/lectures/cross_product_trick.md:133` | Broken `{eq}` reference: `` {eq}`eq:Kalman102} `` (mismatched braces); label `(eq:Kalman102)` is attached to a bare `align*` block on line 121 rather than a numbered `$$ … $$` block | `qe-math-007` / `qe-math-013` (proposed) | Cross-reference cannot bind. |

---

## Consolidated HIGH-priority lectures (29)

29 lectures are HIGH priority because their overall score ≤ 5.0 **or** a single category score ≤ 4.
...
- **`lecture-python-advanced.myst`** (6): the 6 listed in the table above
