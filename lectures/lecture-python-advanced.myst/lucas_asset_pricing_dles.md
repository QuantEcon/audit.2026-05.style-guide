...
### High severity
- **[qe-math-003]** — Every matrix written with `\begin{array}{ccc}...\end{array}` inside `\left[...\right]` instead of `\begin{bmatrix}` (carry-forward from v1 M4). *Example:* `lectures/lucas_asset_pricing_dles.md:89`-`93`, `:99`-`109`, `:113`-`117`, `:121`-`125`, `:129`-`133`. *Count:* ~7 occurrences.
- **[qe-math-002]** — Prime `'` used as transpose throughout the math (carry-forward from v1 M2). *Example:* `lectures/lucas_asset_pricing_dles.md:132`, `:141`, `:147`, `:151`, `:157`. *Count:* ~12 occurrences.
...
## Strengths
- `\mathbb{E}` used for expectation (qe-math-A1) at L63.
