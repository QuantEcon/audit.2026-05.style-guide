...
### High severity
- **[qe-math-A1]** — Bare `E_0`, `E_t`, `E[...]` used throughout instead of `\mathbb{E}` (carry-forward from v1 M7). *Example:* `lectures/additive_functionals.md:951`, `:1007`, `:1029`, `:1098`, `:1294`, `:1296`, `:1318`, `:1327`, `:1400`. *Count:* 12 occurrences.
- **[qe-fig-001]** — `figsize=` set 9 times without justification (qe-fig-001 plus systemic pattern). 

### Medium severity
- **[qe-math-A3]** — `{\cal N}` used as the Normal distribution (carry-forward from v1 M9). *Example:* `lectures/additive_functionals.md:101`, `:103`, `:120`. *Count:* 4 occurrences.
...
### Low severity
- **[qe-fig-003]** — One `ax.set_title` occurrence (carry-forward).
...
## Recommended actions
1. Replace all bare `E` / `E_t` / `E[...]` with `\mathbb{E}` / `\mathbb{E}_t` (qe-math-A1).
