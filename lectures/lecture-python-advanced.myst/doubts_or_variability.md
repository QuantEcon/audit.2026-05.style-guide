...
| Math         | 3/10  | `qe-math-010` (proposed) ×148; `qe-math-011` (proposed) ×24; `qe-math-004` ×6, +1 more. |
...
- **[qe-math-010 (proposed)]** — Blackboard \mathbb{P}, \mathbb{E}, \mathbb{V} with braces. *Count:* 148. *Lines:* 173, 191, 219, 225, 227, 230, 235, 238, 242, 249, …. *Example:* bare expectation `E_t(`.
...
- **[qe-code-002]** — Use Unicode symbols for Greek letters in code. *Count:* 3. *Lines:* 1885, 1886, 1887. *Example:* spelled-out `eta`.
...
1. Brace the expectation and probability operators - 128 occurrences of bare `E_t(`, `E[`, `\Pr` and friends starting at 173, 191, 219, 225, 227, 235, 242, 249, 257, 265 - as `\mathbb{E}` / `\mathbb{P}`; at 128 hits this is by far the largest fix in the file and the reason Math scores 3/10 (qe-math-010 (proposed)).
2. Replace the 24 `\mathcal{N}` distribution names with plain `N` at 441, 459, 918, 975, 977, 2282, 2590, 2606, 2633, 2647 and 14 more (qe-math-011 (proposed)).
...
5. Write the four detection-error probabilities as events in braces - `\Pr_A\{L_T < 0\}` at 1068 and 2906, `\Pr_B\{L_T > 0\}` at 1070 and 2916 (qe-math-014 (proposed)).
...
7. Sweep the small items: `\mathbf{1}` at 265, 270, 2193, 2195 and 2206 should be `\mathbb{1}` with the ones-vector convention spelled out where it first appears, as `math.md` asks (qe-math-004, qe-math-008; 6 occurrences); `iid` to `IID` at 984, 2861, 3013 (qe-writing-009 (proposed)); the apostrophe transpose `W'` at 2537 to `W^\top` (qe-math-002); split the two-sentence paragraphs at 2352, 2582, 2778 and 3115 (qe-writing-001); and strip the trailing whitespace on 38 lines, including the three code lines at 141, 152 and 1342.
