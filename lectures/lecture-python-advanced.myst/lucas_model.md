...
### Low severity
- **[qe-math-A5]** — `'$p*(y)$'` (L500) in a matplotlib label uses `*` next to `p` in a math context (carry-forward from v1 M11). *Count:* 1 occurrence.
- **[qe-fig-005]** — Figures lack `:name: fig-...` fields.
- **[qe-fig-002]** — 1 static `{figure}` (`solution_mass_ex2.png`).

## Strengths
- `\mathbb{E}` and `\mathbb{E}_t` used for expectation throughout (qe-math-A1), e.g. L106, L260.
- Sequences in curly brackets `\{y_t\}_{t=0}^{\infty}` (qe-math-005).
- "IID" used in text correctly (qe-writing-A1) at L86, L385.
...
## Recommended actions
1. Use `p^*` (or `p^{*}`) consistently rather than the literal `*` in plot label (qe-math-A5).
